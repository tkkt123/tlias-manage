...
watch(
  ...
  (newValue, oldValue) => {
     if(newValue.length == 2){
      searchEmp.value.entryStartDate = newValue[0]
      searchEmp.value.entryEndDate = newValue[1]
     }else {
      searchEmp.value.entryStartDate = ''
      searchEmp.value.entryEndDate = ''
     }
  }
)
...
const handleSearch = async () => {
  ...
  const result = await queryPageApi(searchEmp.value.name, searchEmp.value.gender, searchEmp.value.entryStartDate, searchEmp.value.entryEndDate, currentPage.value, pageSize.value);
  if(result.code){
